...
int main()
{
  ...
  D = (DATA_TYPE*)OAOMalloc(NJ*NL*sizeof(DATA_TYPE));
  E = (DATA_TYPE*)OAOMalloc(NI*NL*sizeof(DATA_TYPE));
  E_GPU = (DATA_TYPE*)OAOMalloc(NI*NL*sizeof(DATA_TYPE));
  ...
  OAODataTrans( E_GPU, StConstrTarget.init(7, 5) );
  GPU__mm2(A, B, C, D, E_GPU);
  
  
  OAODataTrans( E_GPU, StConstrTarget.init(7, 2) );
  update(E_GPU);
  ...
  OAODataTrans( E_GPU, StConstrTarget.init(2, 2) );
  OAOFree(E_GPU);
