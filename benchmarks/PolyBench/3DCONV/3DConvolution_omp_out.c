...
void GPU__conv3D(DATA_TYPE* A, DATA_TYPE* B)
{
  ...
  for (j = 1; j < NJ - 1; ++j)
    ...
    OAOStTrans( B, StConstrTarget.init(5, 4) );
...
int main()
{
  ...
  A = (DATA_TYPE*)OAOMalloc(NI*NJ*NK*sizeof(DATA_TYPE));
  B = (DATA_TYPE*)OAOMalloc(NI*NJ*NK*sizeof(DATA_TYPE));
  B_GPU = (DATA_TYPE*)OAOMalloc(NI*NJ*NK*sizeof(DATA_TYPE));
  ...
  OAODataTrans( B_GPU, StConstrTarget.init(7, 5) );
  GPU__conv3D(A, B_GPU);
  
  OAODataTrans( B_GPU, StConstrTarget.init(7, 2) );
  update(B_GPU);
  ...
  OAODataTrans( B_GPU, StConstrTarget.init(2, 2) );
  OAOFree(B_GPU);
