...
void GPU__conv3D(DATA_TYPE *A, DATA_TYPE *B) {
  ...
  #pragma omp target data map(to: A[0:1073741824]) map(tofrom: B[0:1073741824]) if(!RST_AI1)
  {
...
inline void update(DATA_TYPE *B_GPU) { DATA_TYPE bb = B_GPU[0]; }
...
int main() {
  ...
  if(threshold > NI*NJ*NK*sizeof(DATA_TYPE)){
      ...
      cudaMallocHost((void**)&B_GPU, NI*NJ*NK*sizeof(DATA_TYPE));
  }
