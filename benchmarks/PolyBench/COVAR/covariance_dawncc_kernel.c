...
void kernel_1(DATA_TYPE *data, DATA_TYPE *mean) {
  int i, j, j1, j2;
/* Determine mean of column vectors of input data matrix */
  char RST_AI1 = 0;
  RST_AI1 |= !(((void*) (data + 0) > (void*) (mean + 2049))
  || ((void*) (mean + 0) > (void*) (data + 4198401)));
  #pragma omp target data map(to: data[0:4198401]) map(tofrom: mean[0:2049]) if(!RST_AI1)
  {
  #pragma omp target teams distribute parallel for private(i, j) if(!RST_AI1)
  for (j = 1; j < (M + 1); j++) {
    mean[j] = 0.0;
    for (i = 1; i < (N + 1); i++) {
      mean[j] += data[i * (M + 1) + j];
...
void kernel_2(DATA_TYPE *data, DATA_TYPE *mean) {
  int i, j, j1, j2;
/* Center the column vectors. */
  char RST_AI1 = 0;
  RST_AI1 |= !(((void*) (data + 0) > (void*) (mean + 2049))
  || ((void*) (mean + 0) > (void*) (data + 4198401)));
  #pragma omp target data map(to: mean[0:2049]) map(tofrom: data[0:4198401]) if(!RST_AI1)
  {
  #pragma omp target teams distribute parallel for private(i, j) if(!RST_AI1)
  for (i = 1; i < (N + 1); i++) {
    for (j = 1; j < (M + 1); j++) {
...
void kernel_3(DATA_TYPE *data, DATA_TYPE *symmat) {
  int i, j, j1, j2;
/* Calculate the m * m covariance matrix. */
  char RST_AI1 = 0;
  RST_AI1 |= !(((void*) (data + 0) > (void*) (symmat + 4198401))
  || ((void*) (symmat + 0) > (void*) (data + 4198401)));
  #pragma omp target data map(to: data[0:4198401]) map(tofrom: symmat[0:4198401]) if(!RST_AI1)
  {
  #pragma omp target teams distribute parallel for private(j1, j2, i) if(!RST_AI1)
  for (j1 = 1; j1 < (M + 1); j1++) {
    for (j2 = j1; j2 < (M + 1); j2++) {
      symmat[j1 * (M + 1) + j2] = 0.0;
      for (int i = 1; i < N + 1; i++) {
