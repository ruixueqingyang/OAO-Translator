...
void kernel_1(DATA_TYPE *_fict_, DATA_TYPE *ey, int t) {
  ...
  char RST_AI1 = 0;
  RST_AI1 |= !(((void*) (_fict_ + 0) > (void*) (ey + 4095))
  || ((void*) (ey + 0) > (void*) (_fict_ + 999)));
  #pragma omp target data map(to: _fict_[0:1000]) map(tofrom: ey[0:4096]) if(!RST_AI1)
  {
  #pragma omp target parallel for private(j) if(!RST_AI1)
  for (j = 0; j < NY; j++) {
    ey[0 * NY + j] = _fict_[t];
  }
  }
...
void kernel_2(DATA_TYPE *ey, DATA_TYPE *hz) {
  ...
  RST_AI1 |= !(((void*) (ey + 0) > (void*) (hz + 16777215))
  || ((void*) (hz + 0) > (void*) (ey + 16777215)));
  #pragma omp target data map(to: hz[0:16777216]) map(tofrom: ey[0:16777216]) if(!RST_AI1)
  {
  #pragma omp target parallel for private(i, j) collapse(2) if(!RST_AI1)
  for (i = 1; i < NX; i++) {
    for (j = 0; j < NY; j++) {
      ey[i * NY + j] =
          ey[i * NY + j] - 0.5 * (hz[i * NY + j] - hz[(i - 1) * NY + j]);
...
void kernel_3(DATA_TYPE *ex, DATA_TYPE *hz) {
  ...
  char RST_AI1 = 0;
  RST_AI1 |= !(((void*) (ex + 0) > (void*) (hz + 16777215))
  || ((void*) (hz + 0) > (void*) (ex + 16781310)));
  #pragma omp target data map(to: hz[0:16777216]) map(tofrom: ex[0:16781311]) if(!RST_AI1)
  {
  #pragma omp target parallel for private(i, j) collapse(2) if(!RST_AI1)
  for (i = 0; i < NX; i++) {
    for (j = 1; j < NY; j++) {
...
void kernel_4(DATA_TYPE *ex, DATA_TYPE *ey, DATA_TYPE *hz) {
  ...
  char RST_AI1 = 0;
  RST_AI1 |= !(((void*) (ex + 0) > (void*) (ey + 16781311))
  || ((void*) (ey + 0) > (void*) (ex + 16781311)));
  RST_AI1 |= !(((void*) (ex + 0) > (void*) (hz + 16777215))
  || ((void*) (hz + 0) > (void*) (ex + 16781311)));
  RST_AI1 |= !(((void*) (ey + 0) > (void*) (hz + 16777215))
  || ((void*) (hz + 0) > (void*) (ey + 16781311)));
  #pragma omp target data map(to: ex[0:16781312],ey[0:16781312]) map(tofrom: hz[0:16777216]) if(!RST_AI1)
  {
  #pragma omp target parallel for private(i, j) collapse(2) if(!RST_AI1)
  for (i = 0; i < NX; i++) {
    for (j = 0; j < NY; j++) {
