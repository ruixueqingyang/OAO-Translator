...
// Determine mean of column vectors of input data matrix
  char RST_AI1 = 0;
  RST_AI1 |= !(((void*) (data + 0) > (void*) (mean + 1024))
  || ((void*) (mean + 0) > (void*) (data + 1050624)));
  #pragma omp target data map(to: data[0:1050625]) map(tofrom: mean[0:1025]) if(!RST_AI1)
  {
  #pragma omp target parallel for collapse(1) if(!RST_AI1)
  for (j = 1; j < (M + 1); j++) {
    mean[j] = 0.0;
    for (i = 1; i < (N + 1); i++) {
      mean[j] += data[i * (M + 1) + j];
...
// Calculate the m * m correlation matrix.
  char RST_AI4 = 0;
  RST_AI4 |= !(((void*) (data + 0) > (void*) (symmat + 1050624))
  || ((void*) (symmat + 0) > (void*) (data + 1050624)));
  #pragma omp target data map(to: data[0:1050625]) map(tofrom: symmat[0:1050625]) if(!RST_AI4)
  {
  #pragma omp target parallel for collapse(1) if(!RST_AI4)
  for (k = 1; k < M; k++) {
    symmat[k * (M + 1) + k] = 1.0;
    for (j = k + 1; j < (M + 1); j++) {
      symmat[k * (M + 1) + j] = 0.0;
