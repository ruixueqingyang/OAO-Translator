...
// Determine mean of column vectors of input data matrix
#pragma omp target data map( tofrom: data[:Length1], mean[:Length2] )
{
...
// Calculate the m * m correlation matrix.
#pragma omp target data map( tofrom: data[:Length1], symmat[:Length1] )
{
...
int main()
{
  ...
  double s1, s2, e1, e2, s3, s4, e3, e4;;
  ...
  if(threshold > (M + 1) * (N + 1)*sizeof(DATA_TYPE)){
      ...
      cudaMallocHost((void**)&symmat_GPU, (M + 1) * (N + 1)*sizeof(DATA_TYPE));
  }
  ...
  // printf("Host to Device number of byte is: %ld\n", (3*Length1+2*Length2)*sizeof(DATA_TYPE));
  // printf("Host to Device time is: %0.6lfs\n", 2*(e1 - s1)+(e2 - s2));
  
  // printf("Device to Host number of times is: %d\n", 5);
  // printf("Device to Host number of byte is: %ld\n", (2*Length1+3*Length2)*sizeof(DATA_TYPE));
  // printf("Device to Host time is: %0.6lfs\n", 2*(e3 - s3)+(e4 - s4));
