...
// Determine mean of column vectors of input data matrix
#pragma omp target teams distribute parallel for map(tofrom: j, i) collapse(1)
  for (j = 1; j < (M + 1); j++)
  {
    mean[j] = 0.0;
    for (i = 1; i < (N + 1); i++)
...
#pragma omp target teams distribute parallel for map(tofrom: k, j) collapse(1)
  for (k = 1; k < M; k++)
  {
    symmat[k * (M + 1) + k] = 1.0;
    for (j = k + 1; j < (M + 1); j++)
  ...
  OAODataTrans( symmat, StConstrTarget.init(7, 2) );
    symmat[M * (M + 1) + M] = 1.0;
  return;
  OAOStTrans( symmat, StConstrTarget.init(3, 2) );
...
int main()
{
  ...
  stddev = (DATA_TYPE *)OAOMalloc((M + 1) * sizeof(DATA_TYPE));
  symmat = (DATA_TYPE *)OAOMalloc((M + 1) * (N + 1) * sizeof(DATA_TYPE));
  symmat_GPU = (DATA_TYPE *)OAOMalloc((M + 1) * (N + 1) * sizeof(DATA_TYPE));
  ...
  OAODataTrans( symmat_GPU, StConstrTarget.init(7, 5) );
  GPU__correlation(data, mean, stddev, symmat_GPU);
  
  OAODataTrans( symmat_GPU, StConstrTarget.init(7, 2) );
  t_end = rtclock();
  ...
  OAODataTrans( symmat_GPU, StConstrTarget.init(2, 2) );
  OAOFree(symmat_GPU);
