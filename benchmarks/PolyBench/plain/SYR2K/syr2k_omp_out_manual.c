...
#pragma omp target data map(tofrom : A[:Length1], B[:Length1], C[:Length1])
    {
#pragma omp target teams distribute parallel for collapse(1) //map(tofrom : i, j, k)
// #pragma omp target teams num_teams(128)
// #pragma omp distribute parallel for num_threads(64)  collapse(1)
        for (i = 0; i < N; i++) {
            for (j = 0; j < N; j++) {
                for (k = 0; k < M; k++) {
                    C[i * N + j] += ALPHA * A[i * M + k] * B[j * M + k];
