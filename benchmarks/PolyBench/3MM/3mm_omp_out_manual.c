...
void GPU__mm3(DATA_TYPE *A, DATA_TYPE *B, DATA_TYPE *C, DATA_TYPE *D, DATA_TYPE *E, DATA_TYPE *F, DATA_TYPE *G)
{
  ...
  #pragma omp target data map( tofrom: A[:Length], B[:Length],  E[:Length] )
  {
  /* E := A*B */
  ...
  #pragma omp target data map( tofrom: C[:Length], D[:Length], F[:Length] )
  {
  /* F := C*D */
  #pragma omp target teams distribute parallel for map(tofrom: i, j, k) collapse(2)
  for (i = 0; i < NJ; i++)
  ...
  /* G := E*F */
  #pragma omp target data map( tofrom: E[:Length], F[:Length], G[:Length] )
  {
  #pragma omp target teams distribute parallel for map(tofrom: i, j, k) collapse(2)
  for (i = 0; i < NI; i++)
    {
