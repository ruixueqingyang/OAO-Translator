...
void GPU__bicg(DATA_TYPE* A, DATA_TYPE* r, DATA_TYPE* s, DATA_TYPE* p, DATA_TYPE* q)
{
  ...
  OAODataTrans( s, StConstrTarget.init(7, 2) );
  for (i = 0; i < NY; i++)
    {
      s[i] = 0.0;
    }
    OAOStTrans( s, StConstrTarget.init(3, 2) );
    

  {
  OAODataTrans( A, StConstrTarget.init(7, 5) );
  OAODataTrans( r, StConstrTarget.init(7, 5) );
  OAODataTrans( s, StConstrTarget.init(7, 5) );
	  ...
	  OAOStTrans( s, StConstrTarget.init(5, 4) );
	  


OAODataTrans( p, StConstrTarget.init(7, 5) );

OAODataTrans( q, StConstrTarget.init(7, 5) );
	   ...
	   OAOStTrans( q, StConstrTarget.init(5, 4) );
...
int main()
{
  ...
  r = (DATA_TYPE*)OAOMalloc(NX*sizeof(DATA_TYPE));
  s = (DATA_TYPE*)OAOMalloc(NY*sizeof(DATA_TYPE));
  p = (DATA_TYPE*)OAOMalloc(NY*sizeof(DATA_TYPE));
  q = (DATA_TYPE*)OAOMalloc(NX*sizeof(DATA_TYPE));
  s_GPU = (DATA_TYPE*)OAOMalloc(NY*sizeof(DATA_TYPE));
  q_GPU = (DATA_TYPE*)OAOMalloc(NX*sizeof(DATA_TYPE));


  printf("<<OAO BiCG Sub Kernel of BiCGStab Linear Solver >>\n");
  ...
  OAODataTrans( r, StConstrTarget.init(7, 5) );
  OAODataTrans( p, StConstrTarget.init(7, 5) );
  OAODataTrans( q_GPU, StConstrTarget.init(7, 5) );
  GPU__bicg(A, r, s_GPU, p, q_GPU);
  
  
  OAODataTrans( s_GPU, StConstrTarget.init(7, 2) );
  OAODataTrans( q_GPU, StConstrTarget.init(7, 2) );
  update(s_GPU, q_GPU);
  ...
  OAODataTrans( r, StConstrTarget.init(2, 2) );
  OAOFree(r);
  OAOFree(s);
  
  OAODataTrans( p, StConstrTarget.init(2, 2) );
  OAOFree(p);
  OAOFree(q);
  
  OAODataTrans( s_GPU, StConstrTarget.init(2, 2) );
  OAOFree(s_GPU);
  
  OAODataTrans( q_GPU, StConstrTarget.init(2, 2) );
  OAOFree(q_GPU);
