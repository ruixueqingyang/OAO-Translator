...
void GPU__bicg(DATA_TYPE* A, DATA_TYPE* r, DATA_TYPE* s, DATA_TYPE* p, DATA_TYPE* q)
{
  ...
  int Length1 = NX*NY;
  int Length2 = NX;
    //#pragma omp target update to( s[:length1] )

  #pragma omp target data map( tofrom: A[:Length1], r[:Length2], s[:Length2] )
  {
  ...
  #pragma omp target data map( tofrom: A[:Length1], p[:Length2], q[:Length2] )
  {
  ...
  //#pragma omp target update from(A[:Length1],p[:Length2],q[Length2],s[Length2],r[:Length2])
...
int main()
{
  ...
  printf("<<Manual BiCG Sub Kernel of BiCGStab Linear Solver >>\n");
  printf("NX: %d\n", NX);
  printf("NY: %d\n", NY);
