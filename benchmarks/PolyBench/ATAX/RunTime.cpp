...
#ifdef EXPENSES
    gettimeofday(&timeEnd, NULL);
    runTime = (timeEnd.tv_sec - timeStart.tv_sec - FreeTimeEnd.tv_sec + FreeTimeStart.tv_sec) + (double)(timeEnd.tv_usec - timeStart.tv_usec - FreeTimeEnd.tv_usec + FreeTimeStart.tv_usec) / 1000000;
    OAOtime += runTime;
#endif
    return;
}
...
void OAODeleteMallocInfo(void *ptr) {
    ...
    int index = InWhinchMem(ptr);
    if (index >= 0 && OAOMemEnv[index].MallocType==0) {
        OAOMemEnv.erase(OAOMemEnv.begin() + index);
    }
#ifdef EXPENSES
    gettimeofday(&timeEnd, NULL);
...
void OAOExpenseTime(){
    ...
    //因为加了运行时，所产生的时间消耗
    std::cout << "Expense Time(s) is: " << std::dec << OAOtime + expenseTime - D2H_time - H2D_time - Del_time << std::endl;
    // std::cout << "OAOtime Time(s) is: " << std::dec << OAOtime << std::endl;
