...
int main()
{
  ...
  A = (DATA_TYPE*)OAOMalloc(NI*NJ*sizeof(DATA_TYPE));
  B = (DATA_TYPE*)OAOMalloc(NI*NJ*sizeof(DATA_TYPE));
  B_OMP = (DATA_TYPE*)OAOMalloc(NI*NJ*sizeof(DATA_TYPE));
  ...
  OAODataTrans( B_OMP, StConstrTarget.init(7, 5) );
  GPU__conv2D(A, B_OMP);
  
  OAODataTrans( B_OMP, StConstrTarget.init(7, 2) );
  update(B_OMP);
  ...
  OAODataTrans( B_OMP, StConstrTarget.init(2, 2) );
  OAOFree(B_OMP);
