...
{
    #pragma omp target data map( tofrom: variables[:nelr*NVAR] )
    {
...
{
    #pragma omp target data map( tofrom: variables[:nelr*NVAR], fc_momentum_x[:nelr*NDIM], fc_momentum_y[:nelr*NDIM], fc_momentum_z[:nelr*NDIM], fc_density_energy[:nelr*NDIM] )
    {
	#pragma omp target teams distribute parallel for
	for(int i = 0; i < nelr; i++)
	{
...
void compute_flux(int nelr, int* elements_surrounding_elements, float* normals, float* variables, float* fc_momentum_x, float* fc_momentum_y, float* fc_momentum_z, float* fc_density_energy, float* fluxes)
{
    ...
    #pragma omp target data map( tofrom: elements_surrounding_elements[:nelr*NNB], normals[:NDIM*NNB*nelr], variables[:nelr*NVAR], fluxes[:nelr*NVAR], fc_momentum_x[:nelr*NDIM], fc_momentum_y[:nelr*NDIM], fc_momentum_z[:nelr*NDIM], fc_density_energy[:nelr*NDIM] )
    {
	#pragma omp target teams distribute parallel for
	for(int i = 0; i < nelr; i++)
	{
