...
func_ret_t create_matrix_from_file(float **mp, const char *filename, int *size_p) {
    ...
    m = (float *)OAOMalloc(sizeof(float) * size * size);
    if (m == NULL) {
        fclose(fp);
        return RET_FAILURE;
    }
...
func_ret_t create_matrix_from_random(float **mp, int size) {
    ...
    l = (float *)OAOMalloc(size * size * sizeof(float));
    if (l == NULL)
        return RET_FAILURE;

    u = (float *)OAOMalloc(size * size * sizeof(float));
    if (u == NULL) {
        OAOFree(l);
        return RET_FAILURE;
    }
...
func_ret_t lud_verify(float *m, float *lu, int matrix_dim) {
    int i, j, k;
    float *tmp = (float *)OAOMalloc(matrix_dim * matrix_dim * sizeof(float));
...
func_ret_t create_matrix(float **mp, int size) {
    ...
    int i, j;
    float lamda = -0.001;
    float coe[2 * size - 1]; OAOArrayInfo( (void*)(coe), sizeof(float [2 * size - 1]), sizeof(float ) );
    float coe_i = 0.0;
...
void lud_diagonal_omp(float *a, int size, int offset) {
    ...
    OAOStTrans( a, StConstrTarget.init(3, 2) );
...
#pragma omp target map(to : size) map(a [0:size * size])
    ...
    omp_set_num_threads(omp_num_threads);
        ...
        for (offset = 0; offset < size - BS; offset += BS) {
            ...
            OAODataTrans( a, StConstrTarget.init(7, 2) );
...
#pragma #pragma omp target teams distribute parallel for  private(chunk_idx) shared(size, chunks_per_inter, chunks_in_inter_row, offset, a)
...
#pragma omp simd
            ...
            OAOStTrans( a, StConstrTarget.init(5, 4) );
...
int main(int argc, char *argv[]) {
OAOArrayInfo( (void*)(long_options), sizeof(struct option [4]), sizeof(struct option ) );
    ...
    if (do_verify) {
        ...
        OAODataTrans( m, StConstrTarget.init(7, 2) );
    ...
    OAODataTrans( m, StConstrTarget.init(2, 2) );
        OAOFree(m);
