...
T* alloc(int N)
{
	return (T*)OAONewInfo( (void*)(new T[N]), sizeof(T), (N) );
}

template <typename T>
void dealloc(T* array)
{
	delete[] array; OAODeleteInfo( (void*)(array) );
}
...
void copy(T* dst, T* src, int N)
{
	...
	OAOStTrans( dst, StConstrTarget.init(5, 4) );
...
void dump(float* variables, int nel, int nelr)
{
	...
	{
#ifdef OFFLOADING
        std::ofstream file("pre_density_offloading");
#else
        std::ofstream file("pre_density_omp_cpu");
#endif
	...
	{
#ifdef OFFLOADING
        std::ofstream file("pre_momentum_offloading");
#else
        std::ofstream file("pre_momentum_omp_cpu");
#endif
		file << nel << " " << nelr << std::endl;
		for(int i = 0; i < nel; i++)
		{
	...
	{
#ifdef OFFLOADING
        std::ofstream file("pre_density_energy_offloading");
#else
        std::ofstream file("pre_density_energy_omp_cpu");
#endif
		file << nel << " " << nelr << std::endl;
...
float_3 ff_fc_momentum_x;
float_3 ff_fc_momentum_y;
float_3 ff_fc_momentum_z;
float_3 ff_fc_density_energy;
...
void initialize_variables(int nelr, float* variables)
{
	...
	OAOStTrans( variables, StConstrTarget.init(5, 4) );
	
}

inline void compute_flux_contribution(float& density, float_3& momentum, float& density_energy, float& pressure, float_3& velocity, float_3& fc_momentum_x, float_3& fc_momentum_y, float_3& fc_momentum_z, float_3& fc_density_energy)
{
...
inline void compute_velocity(float& density, float_3& momentum, float_3& velocity)
{
...
inline float compute_speed_sqd(float_3& velocity)
{
...
void compute_step_factor(int nelr, float* variables, float* areas, float* step_factors)
{
	...
	for(int i = 0; i < nelr; i++)
	{
		...
		float_3 velocity;	   compute_velocity(density, momentum, velocity);
	...
	OAOStTrans( step_factors, StConstrTarget.init(5, 4) );
...
void compute_flux_contributions(int nelr, float* variables, float* fc_momentum_x, float* fc_momentum_y, float* fc_momentum_z, float* fc_density_energy)
{
	...
	for(int i = 0; i < nelr; i++)
	{
		...
		float_3 velocity_i;             				compute_velocity(density_i, momentum_i, velocity_i);
		float speed_sqd_i                          = compute_speed_sqd(velocity_i);
		float speed_i                              = sqrtf(speed_sqd_i);
		float pressure_i                           = compute_pressure(density_i, density_energy_i, speed_sqd_i);
		float speed_of_sound_i                     = compute_speed_of_sound(density_i, pressure_i);
		float_3 fc_i_momentum_x, fc_i_momentum_y, fc_i_momentum_z;
		float_3 fc_i_density_energy;	
	...
	OAOStTrans( fc_momentum_x, StConstrTarget.init(5, 4) );
	OAOStTrans( fc_momentum_y, StConstrTarget.init(5, 4) );
	OAOStTrans( fc_momentum_z, StConstrTarget.init(5, 4) );
	OAOStTrans( fc_density_energy, StConstrTarget.init(5, 4) );
...
void compute_flux(int nelr, int* elements_surrounding_elements, float* normals, float* variables, float* fc_momentum_x, float* fc_momentum_y, float* fc_momentum_z, float* fc_density_energy, float* fluxes)
{
	...
	{
		int j, nb;
		float_3 normal; float normal_len;
		float factor;
		...
		float_3 momentum_nb;
		float_3 fc_nb_momentum_x, fc_nb_momentum_y, fc_nb_momentum_z;
		float_3 fc_nb_density_energy;
	...
	OAOStTrans( fluxes, StConstrTarget.init(5, 4) );
...
int main(int argc, char** argv)
{
OAOArrayInfo( (void*)(ff_variable), sizeof(float [5]), sizeof(float ) );
	if (argc < 2)
	{
	...
	float* normals;
	{
		...
		areas = (float*)OAONewInfo( (void*)(new float[nelr]), sizeof(float), (nelr) );
		elements_surrounding_elements = (int*)OAONewInfo( (void*)(new int[nelr*NNB]), sizeof(int), (nelr*NNB) );
		normals = (float*)OAONewInfo( (void*)(new float[NDIM*NNB*nelr]), sizeof(float), (NDIM*NNB*nelr) );
		...
		OAOStTrans( elements_surrounding_elements, StConstrTarget.init(3, 2) );
		OAOStTrans( normals, StConstrTarget.init(3, 2) );
		...
		OAOStTrans( areas, StConstrTarget.init(3, 2) );
		OAOStTrans( elements_surrounding_elements, StConstrTarget.init(3, 2) );
		OAOStTrans( normals, StConstrTarget.init(3, 2) );
	...
	OAODataTrans( variables, StConstrTarget.init(7, 5) );
	...
	OAODataTrans( areas, StConstrTarget.init(7, 5) );
	for(int i = 0; i < iterations; i++)
	{

		OAODataTrans( old_variables, StConstrTarget.init(7, 5) );
				copy<float>(old_variables, variables, nelr*NVAR);

		// for the first iteration we compute the time step

		OAODataTrans( variables, StConstrTarget.init(7, 5) );
		OAODataTrans( step_factors, StConstrTarget.init(7, 5) );
				compute_step_factor(nelr, variables, areas, step_factors);

		
		OAODataTrans( elements_surrounding_elements, StConstrTarget.init(7, 5) );
		OAODataTrans( normals, StConstrTarget.init(7, 5) );
		OAODataTrans( variables, StConstrTarget.init(7, 5) );
		OAODataTrans( fluxes, StConstrTarget.init(7, 5) );
		OAODataTrans( fc_momentum_x, StConstrTarget.init(7, 5) );
		OAODataTrans( fc_momentum_y, StConstrTarget.init(7, 5) );
		OAODataTrans( fc_momentum_z, StConstrTarget.init(7, 5) );
		OAODataTrans( fc_density_energy, StConstrTarget.init(7, 5) );
		for(int j = 0; j < RK; j++)
		{
	...
	OAODataTrans( variables, StConstrTarget.init(7, 2) );
	...
	OAODataTrans( areas, StConstrTarget.init(2, 2) );
		dealloc<float>(areas);

	OAODataTrans( elements_surrounding_elements, StConstrTarget.init(2, 2) );
		dealloc<int>(elements_surrounding_elements);

	OAODataTrans( normals, StConstrTarget.init(2, 2) );
		dealloc<float>(normals);


	OAODataTrans( variables, StConstrTarget.init(2, 2) );
		dealloc<float>(variables);

	OAODataTrans( old_variables, StConstrTarget.init(2, 2) );
		dealloc<float>(old_variables);

	OAODataTrans( fluxes, StConstrTarget.init(2, 2) );
		dealloc<float>(fluxes);

	OAODataTrans( step_factors, StConstrTarget.init(2, 2) );
		dealloc<float>(step_factors);

	OAODataTrans( fc_momentum_x, StConstrTarget.init(2, 2) );
		dealloc<float>(fc_momentum_x); 

	OAODataTrans( fc_momentum_y, StConstrTarget.init(2, 2) );
		dealloc<float>(fc_momentum_y);

	OAODataTrans( fc_momentum_z, StConstrTarget.init(2, 2) );
		dealloc<float>(fc_momentum_z);

	OAODataTrans( fc_density_energy, StConstrTarget.init(2, 2) );
