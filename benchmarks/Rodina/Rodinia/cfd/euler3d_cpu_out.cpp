...
//     {
#ifdef OFFLOADING
        std::ofstream file("density_offloading");
#else
        std::ofstream file("density_omp_cpu");
#endif
...
//     {
#ifdef OFFLOADING
        std::ofstream file1("momentum_offloading");
#else
        std::ofstream file1("momentum_omp_cpu");
#endif
        file1 << nel << " " << nelr << std::endl;
        for(int i = 0; i < nel; i++)
        {
            for(int j = 0; j != NDIM; j++) file1 << variables[i + (VAR_MOMENTUM+j)*nelr] << " ";
            file1 << std::endl;
        }
//     }
...
//     {
#ifdef OFFLOADING
        std::ofstream file2("density_energy_offloading");
#else
        std::ofstream file2("density_energy_omp_cpu");
#endif
        file2 << nel << " " << nelr << std::endl;
        for(int i = 0; i < nel; i++) file2 << variables[i + VAR_DENSITY_ENERGY*nelr] << std::endl;
//     }
...
void compute_flux(int nelr, int* elements_surrounding_elements, float* normals, float* variables, float* fluxes, float* ff_variable, float_3 ff_flux_contribution_momentum_x, float_3 ff_flux_contribution_momentum_y, float_3 ff_flux_contribution_momentum_z, float_3 ff_flux_contribution_density_energy)
{
...
int main(int argc, char** argv)
{
    ...
    const char* data_file_name = argv[1];
        ...
        float ff_variable[NVAR]; OAOArrayInfo( (void*)(ff_variable), sizeof(float [5]), sizeof(float ) );
        float_3 ff_flux_contribution_momentum_x, ff_flux_contribution_momentum_y, ff_flux_contribution_momentum_z, ff_flux_contribution_density_energy;
    ...
    OAODataTrans( ff_variable, StConstrTarget.init(7, 5) );
    ...
    std::cout << "Saving solution(oao)..." << std::endl;
    ...
    OAODataTrans( ff_variable, StConstrTarget.init(2, 2) );
