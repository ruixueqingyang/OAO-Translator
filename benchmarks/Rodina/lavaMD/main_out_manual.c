					...
					FOUR_VECTOR* fv)
{
    ...
    int len = dim.boxes1d_arg * dim.boxes1d_arg * dim.boxes1d_arg;
    int length1 = len * NUMBER_PAR_PER_BOX;
    //int len2 = 1;
    #pragma omp target data map( tofrom: par, dim, box[:len], rv[:length1], qv[:length1], fv[:length1] )
    {
