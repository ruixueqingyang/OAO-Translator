...
void nw_optimized(int *input_itemsets, int *output_itemsets, int *referrence, int max_rows, int max_cols, int penalty) {
#ifdef OMP_OFFLOAD
...
#ifdef OPENMP
#pragma omp target teams distribute parallel for shared(input_itemsets, referrence) firstprivate(blk, max_rows, max_cols, penalty)
#endif
...
#pragma omp simd
                    for (int j = 0; j < BLOCK_SIZE + 1; ++j) {
                        input_itemsets_l[i * (BLOCK_SIZE + 1) + j] =
                            input_itemsets[max_cols * (b_index_y * BLOCK_SIZE + i) + b_index_x * BLOCK_SIZE + j];
                    }
                }
                ...
                for (int i = 1; i < BLOCK_SIZE + 1; ++i) {
                    for (int j = 1; j < BLOCK_SIZE + 1; ++j) {
                        input_itemsets_l[i * (BLOCK_SIZE + 1) + j] = maximum(
                            ...
                            input_itemsets_l[i * (BLOCK_SIZE + 1) + j - 1] - penalty, input_itemsets_l[(i - 1) * (BLOCK_SIZE + 1) + j] - penalty);
                    }
                }
...
#pragma omp simd
                    for (int j = 0; j < BLOCK_SIZE; ++j) {
                        input_itemsets[max_cols * (b_index_y * BLOCK_SIZE + i + 1) + b_index_x * BLOCK_SIZE + j + 1] =
                            input_itemsets_l[(i + 1) * (BLOCK_SIZE + 1) + j + 1];
                    }
                }
            }
        }
        OAOStTrans(input_itemsets, StConstrTarget.init(5, 5));
        OAOStTrans(referrence, StConstrTarget.init(7, 5));
...
#pragma omp target
        ...
        OAODataTrans(input_itemsets, StConstrTarget.init(7, 5));
...
void runTest(int argc, char **argv) {
    ...
    OAODataTrans(input_itemsets, StConstrTarget.init(7, 2));
    max_rows = max_rows + 1;
    max_cols = max_cols + 1;
    referrence = (int *)OAOMalloc(max_rows * max_cols * sizeof(int));
    input_itemsets = (int *)OAOMalloc(max_rows * max_cols * sizeof(int));
    output_itemsets = (int *)OAOMalloc(max_rows * max_cols * sizeof(int));
    ...
    OAOStTrans(input_itemsets, StConstrTarget.init(3, 2));
    ...
    OAODataTrans(input_itemsets, StConstrTarget.init(7, 2));
    OAODataTrans(referrence, StConstrTarget.init(7, 2));
    for (int i = 1; i < max_cols; i++) {
        for (int j = 1; j < max_rows; j++) {
            referrence[i * max_cols + j] = blosum62[input_itemsets[i * max_cols]][input_itemsets[j]];
        }
    }
    OAOStTrans(input_itemsets, StConstrTarget.init(7, 2));
    OAOStTrans(referrence, StConstrTarget.init(3, 2));
    ...
    OAODataTrans(input_itemsets, StConstrTarget.init(7, 5));
    OAODataTrans(referrence, StConstrTarget.init(7, 5));
...
#define TRACEBACK
    ...
    OAOStTrans(input_itemsets, StConstrTarget.init(7, 2));
    OAOStTrans(referrence, StConstrTarget.init(7, 2));

    fclose(fpo);

#endif

    OAODataTrans(referrence, StConstrTarget.init(2, 2));
    OAOFree(referrence);

    OAODataTrans(input_itemsets, StConstrTarget.init(2, 2));
    OAOFree(input_itemsets);
    OAOFree(output_itemsets);
...
////////////////////////////////////////////////////////////////////////////////
int main(int argc, char **argv) {
    OAOArrayInfo((void *)(blosum62), sizeof(int[24][24]), sizeof(int));
    runTest(argc, argv);
