...
{ 
    #pragma omp target data map( tofrom: dst[:N], src[:N] )
    {
...
{
    #pragma omp target data map( tofrom: variables[:nelr*NVAR], ff_variable[:NVAR], nelr )
    {
    #pragma omp target teams distribute parallel for //default(shared) schedule(static)
...
{
    #pragma omp target data map( tofrom: variables[:nelr*NVAR], areas[:nelr], step_factors[:nelr] )
    {
...
void compute_flux(int nelr, int* elements_surrounding_elements, float* normals, float* variables, float* fluxes, float* ff_variable, float_3 ff_flux_contribution_momentum_x, float_3 ff_flux_contribution_momentum_y, float_3 ff_flux_contribution_momentum_z, float_3 ff_flux_contribution_density_energy)
{
    ...
    #pragma omp target data map( tofrom: elements_surrounding_elements[:nelr*NNB], normals[:NDIM*NNB*nelr], variables[:nelr*NVAR], fluxes[:nelr*NVAR], ff_variable[:NVAR], ff_flux_contribution_momentum_x, ff_flux_contribution_momentum_y, ff_flux_contribution_momentum_z, ff_flux_contribution_density_energy )
    {
    #pragma omp target teams distribute parallel for //default(shared) schedule(auto)
        for(int blk = 0; blk < nelr/block_length; ++blk)
        {
...
{
    #pragma omp target data map( tofrom: old_variables[:nelr*NVAR], variables[:nelr*NVAR], step_factors[:nelr], fluxes[:nelr*NVAR] )
    {
    #pragma omp target teams distribute parallel for //default(shared) schedule(auto)
    for(int blk = 0; blk < nelr/block_length; ++blk)
    {
...
int main(int argc, char** argv)
{
    ...
    std::cout << "Saving solution(manual)..." << std::endl;
