...
void compareResults(DATA_TYPE* C, DATA_TYPE* C_outputFromGpu)
{
    ...
    for (i=0; i < NI; i++) 
    {
        for (j=0; j < NJ; j++) 
        {
            if (percentDiff(C[i*NJ + j], C_outputFromGpu[i*NJ + j]) > PERCENT_DIFF_ERROR_THRESHOLD) //匹配
            {
                fail++;
            }
        }
...
int main()
{
    ...
    C = (DATA_TYPE*)OAOMalloc(NI*NJ*sizeof(DATA_TYPE)); 
    C_outputFromGpu = (DATA_TYPE*)OAOMalloc(NI*NJ*sizeof(DATA_TYPE)); 
    
    printf("<< Matrix-multiply C=alpha.A.B+beta.C >>\n");
    printf("Data size NI: %d\n", NI);
    printf("Data size NJ: %d\n", NJ);
    printf("Data size NK: %d\n", NK);
    ...
    OAODataTrans( C_outputFromGpu, StConstrTarget.init(7, 5) );
        GPU__gemm(A, B, C_outputFromGpu);
    

    OAODataTrans( C_outputFromGpu, StConstrTarget.init(7, 2) );
    ...
    OAODataTrans( C_outputFromGpu, StConstrTarget.init(2, 2) );
        OAOFree(C_outputFromGpu); 
